$(function(){
	...
	$('.edit').on('click',function(){
        $('#error_message').hide();
	    $('#myModalLabel').html('修改项目进度');
		var step_id=$.trim($(this).data('id'));
		// alert(role_id);
		...
		$.ajax({
			...
			{
                $('#edit_project').data('step_id',step_id);
                $('#edit_project').data('type','edit');

                $('#modal_step_name').val(msg.name);

                $('.modal_step_status').each(function(){
                ...
                $('#modal_step_name').parent().removeClass('has-error').removeClass('has-success').children('span').remove();
                $('#error_message').hide();
    ...
    $('#edit_project').on('click',function(){
        ...
        var modal_step_name=$('#modal_step_name').val();
        var status=1;
        var _token=$.trim($('#_token').val());
    ...
    $('#add_project').on('click',function(){
        ...
        $('#modal_step_name').val('').parent().removeClass('has-error').removeClass('has-success').children('span').remove();
        $('#error_message').hide();
        $('#myModal').modal('toggle');
    });

    //初始化提示信息
    $('#modal_step_name').on('focus',function(){
